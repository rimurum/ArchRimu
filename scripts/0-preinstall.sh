...
# if the name of the disk selected contains nvme, mmc or mmcblk append the letter p before the partition number
if [[ "${DISK}" =~ "nvme" ]] || [[ "${DISK}" =~ "mmc" ]] || [[ "${DISK}" =~ "mmcblk" ]]; then
    partition2=${DISK}p2
    partition3=${DISK}p3
else
    partition2=${DISK}2
...
cp -R ${SCRIPT_DIR} /mnt/root/ArchRimu
